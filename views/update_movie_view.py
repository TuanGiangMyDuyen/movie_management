...
class UpdateMovieView:
    def __init__(self, admin_view):
        self.root = admin_view.root
        ...
        self.frame_win = Frame(self.update_win, bg="white")
        ...
        img_button = Button(left_frame, width=15, text="ADD", font=("Arial", 9, "bold"), bg="#F4631E",
                            activebackground="#F0BB78", bd=0, command=self.update_poster)
        ...
        # Frame chứa tiêu đề
        title_frame = Frame(right_frame, bg="#F3F3E0")
        title_frame.pack(fill="x")

        # Tiêu đề
        Label(title_frame, text="CHỈNH SỬA THÔNG TIN BỘ PHIM", font=("Arial", 10, "bold"), bg="#F4631E").pack(fill="x")

        # Tạo Canvas và Scrollbar để chứa các Entry
        canvas = Canvas(right_frame, bg="#F3F3E0")
        scrollbar = ttk.Scrollbar(right_frame, orient="vertical", command=canvas.yview)
        add_frame = Frame(canvas, bg="#F3F3E0")  # Frame chứa các Entry
        ...
        canvas_frame = canvas.create_window((0, 0), window=add_frame, anchor="nw")
        ...
        self.name_entry.insert(0, self.admin_view.movie['name'])
        ...
        self.genre_entry.insert(0, self.admin_view.movie['genre'])
        ...
        self.director_entry.insert(0, self.admin_view.movie['director'])
        ...
        self.actors_entry.insert(0, ", ".join(self.admin_view.movie['actors']))
        ...
        self.language_entry.insert(0, self.admin_view.movie['language'])
        ...
        self.duration_entry.insert(0, str(self.admin_view.movie['duration_minutes']))
        ...
        self.price_entry.insert(0, str(self.admin_view.movie['ticket_price']))
        ...
        self.description_entry.insert(0, self.admin_view.movie['description'])
        ...
        self.rated_entry.insert(0, self.admin_view.movie['rated'])
        ...
        for i, status in enumerate(self.STATUS):
            if status == self.admin_view.movie['status']:
                self.status_combobox.current(i)
                break

        # Ngày chiếu
        Label(add_frame, text="Ngày chiếu: ", font=label_font, bg="#F3F3E0").grid(row=11, column=0, padx=5, pady=5,
                                                                                  sticky="w")
        ...
        for i, room in enumerate(self.ROOMS):
            if room == self.admin_view.movie['room']:
                self.room_combobox.current(i)
                break
        ...
        self.showtime_type_entry.insert(0, self.admin_view.movie['showtime_type'])

        # Suất chiếu
        Label(add_frame, text="Suất chiếu: ", font=label_font, bg="#F3F3E0").grid(row=15, column=0, padx=5, pady=5,
                                                                                  sticky="w")
        ...
        for i, time in enumerate(self.SHOW_TIMES):
            if time == self.admin_view.movie['showtimes']['start_time']:
                self.showtime_combobox.current(i)
                break
        ...
        self.total_seats_entry.insert(0, str(self.admin_view.movie['total_seats']))
        ...
        self.available_seats_entry.insert(0, str(self.admin_view.movie['available_seats']))

        # Nút Lưu
         # Đặt nút với sticky="ew" để kéo giãn theo chiều ngang nhưng vẫn canh giữa
        Button(add_frame,width=53, text="Xác nhận", font=("Arial", 10, "bold"), bg="#F4631E", fg="white", command=self.update_moive,activebackground="#E78B48",bd=0,activeforeground="white").grid(row=18, column=0, columnspan=2, pady=10,padx=10, sticky="ew")
    ...
    def update_moive(self):
        movieInfo = {
            ...
            "poster_url": getattr(self, "filepath", self.admin_view.movie['poster_url'])
        }

        movie_controller = MovieController(movieInfo)
        isValid, message = movie_controller.update_movie(self.admin_view.movie['id'])
        if not isValid:
